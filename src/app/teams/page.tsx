        ...
        <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 py-3 sm:py-0 flex justify-between items-center sm:h-24">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <div className="hidden sm:block">
              <h1 className="text-xl sm:text-2xl font-black bg-linear-to-r from-blue-400 to-cyan-400 bg-clip-text text-transparent">Team Management</h1>
      ...
      <main className="relative z-10 max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 py-6 sm:py-12">
        ...
        <section className="mb-8 sm:mb-12">
          <div className="relative overflow-hidden rounded-xl border border-cyan-500/50 bg-linear-to-br from-slate-900/90 to-slate-800/70 p-4 sm:p-8">
            <h2 className="text-2xl sm:text-3xl font-bold text-white mb-6 sm:mb-8">🏗️ Create New Team</h2>
            <form onSubmit={handleCreateTeam} className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                <div>
                  <label className="block text-xs sm:text-sm font-bold text-cyan-300 mb-2 sm:mb-3">Team Name</label>
                  <input
                    type="text"
                    value={newTeamName}
                ...
                <div>
                  <label className="block text-xs sm:text-sm font-bold text-cyan-300 mb-2 sm:mb-3">Short Code</label>
                  <input
                    type="text"
                    value={newTeamCode}
        ...
        {/* Teams List */}
        <section>
          <h2 className="text-2xl sm:text-4xl font-black text-white mb-6 sm:mb-8">📋 Your Teams</h2>
          {loading ? (
            <div className="text-center text-slate-400 py-12 sm:py-16 text-base sm:text-lg">Loading teams...</div>
          ) : teams.length === 0 ? (
            <div className="relative overflow-hidden rounded-xl border border-cyan-500/50 bg-linear-to-br from-slate-900/90 to-slate-800/70 p-8 sm:p-16 text-center">
              ...
              <p className="text-slate-400 text-base sm:text-lg">No teams yet. Create your first team above!</p>
            </div>
          ) : (
                      ...
                      <div className="flex items-start justify-between mb-4">
                        <div>
                          <h3 className="text-lg sm:text-2xl font-bold text-white">
                            {team.name}
                            {team.shortCode && <span className="text-xs sm:text-sm text-cyan-400 ml-2">({team.shortCode})</span>}
                          </h3>
                        </div>
                      </div>
