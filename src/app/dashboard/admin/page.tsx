...
export default function AdminDashboard() {
  ...
  const [maintenanceMessage, setMaintenanceMessage] = useState(
    "System maintenance is in progress. Please check back soon.",
  );
  ...
  const toggleMaintenanceMode = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        alert(
          `Maintenance mode ${data.isEnabled ? "enabled" : "disabled"} successfully!`,
        );
      } else {
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-0 flex justify-between items-center h-full">
          <div className="flex items-center space-x-3">
            <img
              src="/logo.png"
              alt="CricKeters"
              className="h-32 w-32 object-contain -my-4"
            />
            <div>
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-cyan-500/30">
                      <th className="px-6 py-4 text-left text-sm font-bold text-cyan-300">
                        Name
                      </th>
                      <th className="px-6 py-4 text-left text-sm font-bold text-cyan-300">
                        Email
                      ...
                      <th className="px-6 py-4 text-center text-sm font-bold text-cyan-300">
                        Actions
                      </th>
                    </tr>
                  </thead>
                        ...
                        <td className="px-6 py-4 text-center">
                          {user.id !== adminId ? (
                            ...
                            <div className="flex gap-2 justify-center items-center text-xs">
                              <span className="px-3 py-1 bg-cyan-500/20 border border-cyan-400/50 text-cyan-300 rounded font-semibold">
                                {user.role}
                              </span>
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                ...
                <div className="text-center">
                  ...
                  <h4 className="text-4xl font-bold text-green-400">
                    {users.filter((u) => u.role === "UMPIRE").length}
                  </h4>
                </div>
                <div className="text-center">
                  ...
                  <h4 className="text-4xl font-bold text-emerald-400">
                    {users.length}
                  </h4>
                </div>
              </div>
          ...
          <div className="relative overflow-hidden rounded-xl border border-yellow-500/50 bg-linear-to-br from-slate-900/90 to-slate-800/70 p-8">
            <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6 mb-8">
              <div>
                ...
                <p className="text-yellow-300/70">
                  Enable or disable maintenance mode for all users
                </p>
              </div>
              <div
                className={`px-6 py-3 rounded-lg font-bold text-white text-sm whitespace-nowrap ${maintenanceMode ? "bg-red-600/80" : "bg-green-600/80"}`}
              >
            ...
            {maintenanceMode && (
              <div className="mt-6 bg-yellow-500/10 border border-yellow-400/30 text-yellow-300 p-6 rounded-lg">
                ...
                <p className="text-sm">
                  All users except admins will see the maintenance page. When
                  you disable it, users will be able to access the application
                  again.
                </p>
              </div>
            )}
